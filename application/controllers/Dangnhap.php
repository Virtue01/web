...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='HAD - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->from('hmai.my03@gmail.com', 'HAD');
            $this->email->to($email);
            $this->email->subject('Hệ thống HAD - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng HAD, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 đ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
        ...
        $this->data['title']='HAD - Đăng ký tài khoản';   
...
//laylaipass
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_port']    = '465';
            $config['smtp_timeout'] = '7';
            $config['smtp_user']    = 'thoidaihackgame@gmail.com';
            $config['smtp_pass']    = 'tsqawkzjviorhuyz';
            $config['charset']    = 'utf-8';
            $config['newline']    = "\r\n";
            $config['wordwrap'] = TRUE;
            $config['mailtype'] = 'html';
            $config['validation'] = TRUE;   
            $this->email->initialize($config);
            $this->email->from('HAD', 'HAD');
            $this->email->to($list['email']);
            $this->email->subject('HAD - Lấy lại mật khẩu');
        ...
        $this->data['title']='HAD - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='HAD - Cập nhật mật khẩu mới';
